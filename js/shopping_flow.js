...
//新增判断手机号码位数
var password=document.getElementById("tel_0"); 
...
//新增判断身份证号位数
var password=document.getElementById("sfzh"); 
...
if(password.value.length<18){
  ...
  //新增图片上传判断
  if (Utils.isEmpty(frm.elements['pic1'].value))
  {
    err = true;
    msg.push('身份证正面照片不能为空！');
  }
    if (Utils.isEmpty(frm.elements['pic2'].value))
  {
    err = true;
    msg.push('储蓄卡正面照片不能为空！');
  }
    if (Utils.isEmpty(frm.elements['pic3'].value))
  {
    err = true;
    msg.push('手持身份证照不能为空！');
  }
